    ...
    try {
        ...
        const resources = await Promise.all(uniqueFolders.map(async (folder) => {
            ...
            if (controllerFile) {
                ...
                restMethods = (_a = data === null || data === void 0 ? void 0 : data.match(/@[A-Za-z]+Mapping(\("[^"]*"\))?/g)) === null || _a === void 0 ? void 0 : _a.filter(restCall => {
...
async function hasMany(projectPath, body) {
    ...
    const class1Path = path.join(projectPath, '/src/main/java/com/natetrystuff/', class1);
    const class2Path = path.join(projectPath, '/src/main/java/com/natetrystuff/', class2);
    const class1ObjectFile = readFileSync(class1Path + '/' + class1 + '.java', 'utf8');
